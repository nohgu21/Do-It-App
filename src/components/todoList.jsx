    ...
    <main className="bg-[#1F2937] mt-4 rounded-xl p-4 space-y-6 shadow-lg text-white" aria-label="Todo list section">
      <ul className="space-y-4" role="list">
        {paginatedTodos.map((todo) => (
          <li key={todo.id} role="listitem" className="bg-[#0F4C5C] hover:bg-[#0a3742] px-4 py-3 rounded-xl transition-colors">
            <Link
              to={`/todos/${todo.id}`}
      ...
      <nav className="flex justify-between items-center" role="navigation" aria-label="Pagination Navigation">
        <button
          disabled={currentPage === 1}
          onClick={() => setCurrentPage((prev) => prev - 1)}
          className="btn btn-sm bg-[#0F4C5C] text-white hover:bg-[#0d3a45] disabled:bg-gray-500"
